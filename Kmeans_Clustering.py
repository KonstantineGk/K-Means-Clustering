...
def init_Centroids(X):
    ...
    initial_Centroids = np.reshape( np.concatenate((Centroid0, Centroid1), axis = 0),(2,2) )
...
def update_Centroids(X, Clusters):
    ...
    New_Centroids = np.reshape( np.concatenate( (Centroid0,Centroid1), axis = 0), (2,2) )
...
def kmeans(X):
    ...
    for _ in range(50):                                     
        ...
        Cluster0_metric.append( np.sqrt( (Centroids[0,0]-prev_Centroids[0,0])**2 + (Centroids[0,1]-prev_Centroids[0,1])**2 ) )
        Cluster1_metric.append( np.sqrt( (Centroids[1,0]-prev_Centroids[1,0])**2 + (Centroids[1,1]-prev_Centroids[1,1])**2 ) )
